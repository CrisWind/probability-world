#!/usr/bin/env node
/* Phase C: patch-fleet-runtime.js
 * Creates fleet-runtime.js — round management, persistence, events.
...
console.log('[Phase C] fleet-runtime.js');

const fleetRuntimeContent = `/* 概率世界 · 商会船队风险盘 V0.1 —— 运行时（状态机 + 存档 + 事件）
 * 职责边界：
 *  - 唯一的 fleet 状态持有者（读写 store.world.fleet，不新建 localStorage 入口）；
...
(function attachFleetRuntime(global) {
  ...
  function submitPlan() {
    ...
    if (camp.storms && camp.storms.length > 0) {
      emit('fleet_storm_triggered', {
        campaignId: camp.campaignId,
        round: camp.currentRound,
        storms: camp.storms.map(function(s) { return { routeId: s.routeId, severity: s.severity }; })
      });
    }
...
writeFile('fleet-runtime.js', fleetRuntimeContent, 'FleetRuntime');
verifySyntax('fleet-runtime.js');
